...
describe('DegreeService', () => {
  ...
  describe('getDegrees', () => {
    it('should return an array as a result', async () => {
      ...
      jest
          .spyOn(degreeRepository, 'find')
          .mockImplementation((options: FindManyOptions<Degree>) => {
      ...
      expect(
          await degreeService.getDegrees(criterias, withRecommendedCourses),
      ).toBeInstanceOf(Array);
    });

    it(
        'should make request with criterias and without recommended courses',
        async () => {
          const criterias = {did: 1};
          const withRecommendedCourses = false;
          const result: Degree[] = [];
    ...
    it('should return array as a result', async () => {
      ...
      jest
          ...
          .mockImplementation((options: FindManyOptions<Degree>) => {
            expect(options.where).toEqual(criterias);
            expect(options.relations).toEqual({
              recommendedCourses: {prerequisites: true},
            });
    ...
    it(
        'should make request with criterias and recommended courses',
        async () => {
          const criterias = {did: 1};
          const withRecommendedCourses = true;
          const result: Degree[] = [];
