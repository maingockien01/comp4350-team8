...
import {PersonalRoadmapService}
  from '../../../src/roadmap/personal.roadmap.service';
...
describe('PersonalRoadmapService', () => {
  ...
  describe('getPersonalRoadmap', () => {
    it('should throw for non-existing user', async () => {
      await expect(
          personalRoadmapService.getPersonalRoadmap(0),
      ).rejects.toThrow();
    });
  ...
  describe('savePersonalRoadmap', () => {
    it('should throw for non-existing user', async () => {
      await expect(
          personalRoadmapService.savePersonalRoadmap(0, []),
      ).rejects.toThrow();
    });
    ...
    it('should throw for non-existing courses', async () => {
      const user = await saveUser(app);
      await expect(
          personalRoadmapService.savePersonalRoadmap(user.uid, [
    ...
    it('should save roadmap for user with prerequisites', async () => {
      ...
      const anotherCourse = await saveCourse(app, {prerequisites: [course]});
      const user = await saveUser(app);
      await personalRoadmapService.savePersonalRoadmap(user.uid, [
        anotherCourse.cid,
        course.cid,
      ]);
